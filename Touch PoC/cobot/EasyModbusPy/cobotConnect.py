...
class CobotConnect():
    def __init__(self, ipAddress: str = '192.168.0.1', port: int = 5890):
    ...
    def sendCobotMove(self, point: list, speed: int = 100):
    ...
    def sendCobotPos(self, point: list, speed: int = 100):
        command = "PTP(\"CPP\"," + self.toText(point) + "," + str(speed) + ",200,0,false)"

        self.socket.send(self.packetSender(command).encode())

    def readPos(self) -> list:
    ...
    def chkStr(self, s: str) -> str:
        result = "TMSCT," + str(len(s)+2) + ",3," + s + ","
        return result
    
    def checksum(self, b: bytearray) -> str:
        result = b[0]
    ...
    def packetSender(self, s: str) -> str:
        h=self.chkStr(s)
        b=bytearray(h,'utf-8')
        d=self.checksum(b)
        result="$" + h + "*" + d + "\r\n"
    ...
    def floater(self, x: float, y: float) -> float:
        t = (x,y)
